...
#include "affinity.h"
CHECK1_ACCUM(Branch,next_branch,branch)
CHECK1_ACCUM(Lateral,next_lateral,lateral)
#endif
...
void Compute_Tree(Root r) {
  ...
  tmp.P = 0.0;
  tmp.Q = 0.0;
  /*chatting("Computing on tree 0x%x\n",r);*/
#ifndef FUTURES
  for (i=0; i<NUM_FEEDERS; i++) {
    l = r->feeders[i];
#ifdef VERIFY_AFFINITIES
    Accumulate_lateral(l,l);
#endif
    theta_R = r->theta_R;
    theta_I = r->theta_I;
...
Demand Compute_Lateral(Lateral l, double theta_R, double theta_I, 
  ...
  /*chatting("Computing on lateral 0x%x\n",l);*/
...
Demand Compute_Branch(Branch br, double theta_R, double theta_I, 
  ...
  /*chatting("computing on branch 0x%x\n",br);*/
...
Demand Compute_Leaf(Leaf l, double pi_R, double pi_I) {
  /*chatting("computing on leaf 0x%x\n",l);*/
  P = l->D.P;
  Q = l->D.Q;
