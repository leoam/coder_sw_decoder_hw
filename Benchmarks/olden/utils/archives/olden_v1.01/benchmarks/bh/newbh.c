...
#ifdef VERIFY_AFFINITIES
  chatting("checking tree 0x%x\n",root);
  Docheck_tree((cellptr) root);
#endif
 ...
 bool subdivp(nodeptr p, real dsq, real tolsq, hgstruct hg)
{
  ...
  if (Type(local_p) == BODY)                  /* at tip of tree? */
    return (FALSE);                           /*   then cant subdivide */

  SUBV(dr, Pos(local_p), hg.pos0);               /* compute displacement  */   
...
expandbox(bodyptr p, treeptr t, int nsteps, int proc);
...
{  
 ...
 for (tmp=__NumNodes-1; tmp>=0; tmp--) {
  ...
  node1 = Root(t);
  hackcofm(node1);				/* find c-of-m coordinates  */
  /*CMMD_node_timer_stop(2);*/
...
void dis_number (nodeptr n)
{
  int tnperproc = (int) ceil ( (double) nbody/__NumNodes);
