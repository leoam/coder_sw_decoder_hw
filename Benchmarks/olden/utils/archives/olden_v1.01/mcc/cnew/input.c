...
static void pragma() {
    ...
    else if (strcmp(token, "notest") ==0) 
     {
     warning("#pragma notest no longer supported-- use local type\n");
     }
    else if (strcmp(token, "retest") ==0) 
     {
     warning("#pragma retest no longer supported-- use local type\n");
     }
}
