...
/* Addition by mcc 1/18/95 to check for TESTPs in a tree */
static int anytreetests(tp) Tree tp; {
  if (tp == 0) return 0;
  if (generic(tp->op)==TEST) return 1;
  return (anytreetests(tp->kids[0]) || anytreetests(tp->kids[1]));
}
...
Node listnodes(tp, tlab, flab) Tree tp; {
	...
	case JUMP:
		...
		list(p);
		break;
/**** addition by mcc to support TESTP nodes ****/
   case TEST:
      l = listnodes(tp->kids[0],0,0);
      p = newnode(TESTP,l,0,0);
      p->lineno = tp->lineno;
      p->filename = tp->filename;
      trash(0);
      list(p);
      break;
/**** end of addition by mcc ****/
	case CALL:
		assert(tlab == 0 && flab == 0);
...
/**** modification by mcc to support TESTP nodes ****/
			/*assert(generic(x->op) == INDIR);*/
         assert((generic(x->op) == INDIR) || (generic(x->op)==TEST));
/**** end modification ****/
			trash(0);
         ...
         /* Modification by mcc 1/18/95 to do rhs 1st if it has a test */
         if (anytreetests(tp->kids[1])) {
			  r = listnodes(tp->kids[1], 0, 0);
			  l = listnodes(tp->kids[0], 0, 0);
         }
         else {
      ...
      /* mcc - copy affinity from tree to node */
      /* changed 11/6/95 for voronoi pointer math */
      if (!r || !r->affinity.sym) {
        p->affinity.value = tp->affinity.value;
        p->affinity.direction = tp->affinity.direction;
        p->affinity.sym = tp->affinity.sym;
      }
      else {
        p->affinity.value = r->affinity.value;
        p->affinity.direction = r->affinity.direction;
        p->affinity.sym = r->affinity.sym;
      }
		list(p);
	...
	case BAND:
		...
		if (IR->compl_band)
			...
			r = listnodes(tp->kids[1], 0, 0);
		p = node(tp->op, l, r, 0);
      /* mcc -changed 11/6/95 for voronoi pointer math */
      if (l) {
        p->affinity.sym = l->affinity.sym;
        p->affinity.value = l->affinity.value;
        p->affinity.direction = l->affinity.direction;
        }
		break;
	case RSH:
		assert(tlab == 0 && flab == 0);
	...
	case BOR: case BXOR: case LSH:
      ...
      /* mcc - copy type islocal to node */
      p->islocal = tp->islocal;
...
/**** modification by mcc to support TESTP nodes ****/
			/*assert(p->count == 0);*/
			assert(p->count == 0 || generic(p->op) == TEST || 
                generic(p->op) == MTEST);
/**** end of modification ****/
			undag1(p, p);
			/*
