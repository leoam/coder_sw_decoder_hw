...
static void fields(ty) Type ty; {
   ...
   /* mcc local modification 10/11/94 */
	while (kind[t] == CHAR || t == LOCAL_TYPE
	|| t == ID && tsym && tsym->sclass == TYPEDEF) {
		...
		do {
         ...
         p->affinity = 70; /* mcc default affinity value */
         if (!isptr(fty)) 
           p->direction = 1; /* mcc default direction */
         else
           p->direction = 0; /* has no direction */
         if (t == '{') {
            if (!isptr(fty) && !(isarray(fty) && isptr(fty->type))) 
               error("{affinity} only valid for pointer fields\n");
            t = gettok();
            p->affinity = intexpr(0,0);
            if (isptr(fty))
              expect('}');
            else {
              expect(',');
              p->direction = intexpr(0,0);
              if ((p->direction != 1) && (p->direction != -1))
                 error("affinity direction must be +1 or -1\n");
              expect('}');
              }
            }
			if (t == ':') {
...
for (;;) {
   ...
   /* local modification by mcc 10/4/94 */
	if (t == ID && (tsym == 0 || tsym->sclass != TYPEDEF)
	||  t != ID && t != REGISTER && kind[t] != CHAR && t != LOCAL_TYPE)
...
static Type structdcl(op) {
	...
	if (t == '{') {
      ...
      /* modified for local 10/11/94 mcc */
		if (kind[t] == CHAR || t == LOCAL_TYPE || t == ID && tsym
		&& tsym->sclass == TYPEDEF)
			fields(ty);
		else
...
static Type tnode(op, type) Type type; {
	...
	ty->op = op;
	ty->type = type;
   if (type) ty->islocal = type->islocal;
	return ty;
}
...
static Type type(lev, sclass) int *sclass; {
	...
	for (vol = cons = sign = size = type = 0;;) {
      ...
      case LOCAL_TYPE:
		...
		case STRUCT: case UNION:
			...
			break;
		case ID:
         /* Addition by mcc to support local storage class == 9/19/94 */
			if (tsym && tsym->sclass == TYPEDEF
			&& type == 0 && size == 0 && sign == 0) {
				use(tsym, src);
 
            /* local correction 10/11/94 -- create copy of
               typedef before setting local field */
            if (!islocal || tsym->type->islocal)
				  ty = tsym->type;
            else {
              ty = tnode(tsym->type->op,(Type) 0);
              *ty = *(tsym->type);
            }
        
            /* Correction for local 10/11/94 mcc */
            if (tsym->type->islocal) islocal = 1;
				t = tt = ty->op;
				break;
			}
			/* fall through */
      ...
      /* addition by mcc 9/19/94 to support local storage class */
      if (t == LOCAL_TYPE) {
        t = gettok();
        }
      else {
   ...
   /* Addition by mcc to support local storage class == 9/19/94 */
   if (islocal) ty->islocal = 1; else ty->islocal = 0;
