...
/* mcc addition */
dclproto(Tree skipright,(Tree));

void mccdebug(p) Tree p;
{
}
...
Type assign(xty, p) Type xty; Tree p; {
	...
	if (isptr(xty) && isptr(yty)) {
		...
		if (isstruct(lty) && isstruct(rty) && extends(rty, lty))
			return xty;
      /* local type addition by mcc 10/24/94 */
      if (lty->u.sym == rty->u.sym) return xty;
		if (eqtype(lty, rty, 1)
...
/* find the name of pointer being dereferenced */
/* Added 11/6/95 by mcc */
Symbol find_affinity_sym(p) Tree p; {
   if (p==NULL) return NULL;
   if ((generic(p->op)!=CNST) && p->u.sym) return p->u.sym;
   return find_affinity_sym(p->kids[0]);
}
...
Tree field(p, name) Tree p; char *name; {
   ...
   Tree oldp = p;
	Type ty1, ty = p->type;
   int islocal = p->islocal;

   oldp->affinity.value = -1;
	if (isptr(ty))
		ty = deref(ty);
	ty1 = ty;
	ty = unqual(ty);
	if (q = fieldref(name, ty)) {
      /* mcc affinity addition */
		ty = q->type;
      ...
      p->affinity.value = q->affinity;
      p->affinity.direction = q->direction;
      /* mcc local addition 10/5/94 */
      p->islocal = islocal;
      oldp = skipright(oldp);
      p->affinity.sym = find_affinity_sym(oldp->kids[0]);
      oldp=p;
		if (q->lsb) {
...
/* Added by mcc 1/13/95 */
static int notests(e1) Tree e1; {
  if (!e1) return 1;
  if (e1->op == TESTP) return 0;
  return (notests(e1->kids[0]) && notests(e1->kids[1]));
}

/* incr - construct tree for e1 op= e2 */
/* modified by mcc 1/13/95 to allow a->x op= e2, e.g. */
Tree incr(op, e1, e2) Tree e1, e2; {
   if (notests(e1)) 
	  return asgnnode(ASGN, e1, (*opnode[op])(oper[op], e1, e2));
   else
     return asgnnode(ASGN, e1, (*opnode[op])(oper[op], copytree(e1), e2));
}
...
static Tree nullcheck(p) Tree p; {
   ...
   /* modification by mcc 9/19/94 to support local storage class */
   if (p->islocal) return p;

   /* modification by mcc to add TESTP nodes for null checks */
   q = tree(TESTP,p->type,p,0);
   q->affinity = p->affinity;
   q->lineno = lineno;
   q->filename = file;
   return q;
/**** commented out by mcc 
...
Tree pointer(p) Tree p; {
   ...
   /* local modification 10/4/94 */
   p->islocal = islocal;
	return p;
}

int mccnotest(p) Tree p; {
	...
	for (;;)
		...
		case '[':
         oldp = p;
			t = gettok();
			q = expr(']');
         if (p->type && p->type->islocal) p->islocal = 1;
			if (YYnull)
           {
				if (isptr(p->type))
               {
               /* modified for local 10/4/94 -- mcc */
					p = nullcheck(p);
               }
            else if 
                (((generic(p->op)==ADD) && (isptr(p->kids[0]->type))) ||
                 ((generic(p->op)==INDIR) && (isptr(p->kids[0]->type))))
               {
               /* modified for local 10/11/94 -- mcc */
               /* to fix bug I seperate this from above for array inside 
                  structure pointer */
               p->islocal = p->kids[0]->islocal;
					p = nullcheck(p);
               }
				if (isptr(q->type))
					q = nullcheck(q);
         ...
         /* mcc local addition--10/4/94 */
         p->islocal = oldp->islocal;

			if (isptr(p->type) && isarray(p->type->type))
				p = retype(p, p->type->type);
			else
				p = rvalue(p);
         /* mcc local,affinity additions */
         p->islocal = oldp->islocal;
         p->affinity = oldp->affinity;
			continue;
		case '(': {
		...
		case DEREF:
			...
			if (t == ID) {
            Type oldtype;
            /* mcc addition for local 10/3/94 */
            int islocal=p->type->islocal;

            p->islocal = islocal;
				if (isptr(p->type))
					ty = deref(p->type);
				if (isptr(p->type) && isstruct(ty)) {
            /* modification by mcc to place TESTPs above ADDPs?!? 
               reversed field and YYnull calls */
               oldtype=p->type;
					p = field(p, token);	/* refme */
                  /* modified by mcc from above */
					if (YYnull)
						/*p = nullcheck(p);*/
                 {
                  if(p->kids[0] && ((p->kids[0]->op==ADD+P) ||
                     (p->kids[0]->op==RIGHT) ||
                     (p->kids[0]->op==CVUP) ||
                    ...
                    {
                      if (!islocal) 
						      p->kids[0] = nullcheck(p->kids[0]);
                    }
                  else if (((p->op==ADD+P) && (generic(p->kids[0]->op)==INDIR)
                            && mccnotest(p->kids[0]->kids[0])) ||
                           ((generic(p->op)==INDIR) && 
                               mccnotest(p->kids[0])))
                                        {}
                       /* Don't test if no pointer actually dereferenced */
                  else {
                       p = nullcheck(p);
                       if (p->kids[0]) p->kids[0]->type = oldtype;
                       }
                 }
				} else
					error("left operand of -> has incompatible type `%t'\n",
...
static Tree prefix() {
	...
	case '*':
      ...
      /* mcc local addition 10/4/94 */
      if (p->type->islocal == 1) p->islocal = 1;
		if (YYnull)
			p = nullcheck(p);
		p = rvalue(p);
		break;
	case '&':
		...
		if (isaddrop(p->op))
			if (p->u.sym->sclass == REGISTER)
				...
				p->u.sym->addressed = 1;
      /* mcc addition 11/8/94 for &(p->q) to remove TEST */
      if (generic(p->op) == TEST) p = p->kids[0];
		break;
	case '+':
		t = gettok();
		p = pointer(prefix());
