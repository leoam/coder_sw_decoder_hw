...
#include "c.h"
#define LOCALPTR -1 /* want something that won't conflict with any known 
                       type */
...
Type array(ty, n, a) Type ty; {
   Type retval;
	...
	retval=tynode(ARRAY, ty, n*ty->size, a, (Generic)0);
   if (ty->op==POINTER) 
     retval->islocal = ty->islocal;
   else retval->islocal = 1;
   return retval;
}
...
Type composite(ty1, ty2) Type ty1, ty2; {
	...
	case ARRAY: {
		Type ty;
      /* mcc local addition 10/5/94 -- weird bug in compositing types
         caused by having different local/nonlocal types */
      if ((ty1->size == ty2->size) && (ty1->type==ty2->type)) return ty1;
...
Type ptr(ty) Type ty; {
   /* modified by mcc 9/19/94 to support local storage class */
   Type retval;
   int op;

   if (ty->islocal) op = LOCALPTR; else op = POINTER;
	retval = tynode(op, ty, IR->ptrmetric.size,
...
static Type tynode(op, type, size, align, ptr) Type type; Generic ptr; {
   ...
   /* addition by mcc 9/19/94 for local pointer */
   if (op == LOCALPTR) { op = POINTER; islocal = 1; } else islocal = 0;

	i = (opindex(op)^((unsigned)type>>2))&
       (sizeof typetable/sizeof typetable[0]-1);
	if (op != ARRAY && op != FUNCTION || size > 0)
		for (tn = typetable[i]; tn; tn = tn->link)
			if (tn->type.op   == op   && tn->type.type  == type
         /* addition by mcc 9/19/94 */
         &&  tn->type.islocal == islocal
