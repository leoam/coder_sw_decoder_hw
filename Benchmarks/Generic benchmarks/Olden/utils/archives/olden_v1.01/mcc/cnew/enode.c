   ...
   int islocal;
	...
	if (isarith(l->type) && isarith(r->type)) {
      ...
      islocal = l->islocal || r->islocal;
		...
		retval=simplify(ADD+P, ty, l, r);
      /* mcc local modification 10/4/94 */
      retval->islocal = islocal;
      return retval;
	} else
		typeerror(op, l, r);
	retval=simplify(op, ty, l, r);
   return retval;
}
...
/* mcc addition of skipright to handle casts of pointer assignments */
Tree skipright(r) Tree r; {
   assert(r);
   if (r->op==RIGHT) return skipright(r->kids[1]);
   return r;
}
   ...
   Tree retval;
	...
	if (isstruct(ty)) {
		...
		&& isaddrop(l->op)) {
         ...
         r->kids[0]->lineno = lineno;
         r->kids[0]->filename = file;
			return rvalue(r);
		}
		return tree(ASGN+B, ty, l, r);
	}
	retval=tree(op + (isunsigned(ty) ? I : ttob(ty)), ty, l, r);
   /* mcc addition for affinity */
   if (isptr(ty))
      {
         r = skipright(r);
         retval->affinity = r->affinity;
      }
   else retval->affinity.sym = NULL;
   /*mccdebug();*/
   return retval;
}
...
Tree callnode(f, ty, args) Tree f, args; Type ty; {
	...
	if (isstruct(ty)) {
		...
		p = tree(RIGHT, ty, tree(CALL+B, ty, f, lvalue(idnode(t1))),
			idnode(t1));
      p->kids[0]->lineno = lineno;
      p->kids[0]->filename = file;
	} else {
		Type rty = ty;
		if (isenum(ty))
			rty = unqual(ty)->type;
