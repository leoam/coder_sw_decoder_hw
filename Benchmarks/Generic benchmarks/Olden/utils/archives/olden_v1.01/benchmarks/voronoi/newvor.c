...
#include "affinity.h"
CHECK1_ACCUM(QUAD_EDGE,next,edge)
CHECK2(VERTEX_PTR,left,right,tree)
#endif
...
#endif
#ifdef VERIFY_AFFINITIES
   Accumulate_edge(ldo,ldo);
   Accumulate_edge(rdo,rdo);
#endif
     ...
     int i,seed;
{
  ...
  /*chatting("node = 0x%x\n",node);*/
  tmp = X(node);
  point = get_points(n/2,tmp,i-1,point.seed,
